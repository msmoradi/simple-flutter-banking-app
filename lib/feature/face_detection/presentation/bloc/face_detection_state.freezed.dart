...
abstract class $FaceDetectionStateCopyWith<$Res> {
  ...
  @useResult
  $Res call({FaceDetectionStatus status, String errorMessage, int progress});
}
...
class _$FaceDetectionStateImpl implements _FaceDetectionState {
  const _$FaceDetectionStateImpl(
      {this.status = FaceDetectionStatus.initial,
      this.errorMessage = '',
      this.progress = 0});
  ...
  @override
  String toString() {
    return 'FaceDetectionState(status: $status, errorMessage: $errorMessage, progress: $progress)';
  }
  ...
  @override
  int get hashCode => Object.hash(runtimeType, status, errorMessage, progress);
...
abstract class _FaceDetectionState implements FaceDetectionState {
  const factory _FaceDetectionState(
      {final FaceDetectionStatus status,
      final String errorMessage,
      final int progress}) = _$FaceDetectionStateImpl;
